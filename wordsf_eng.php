<?php
    ...
    function fetchdata($link, $word, &$cache_status)
    {
        ...
        // query and update words
        $query_str = "SELECT id FROM wordseng WHERE word='$slashed_word' LIMIT 1";
        $result = mysql_query($query_str, $link);
        $row = mysql_fetch_array( $result );
        if ($row) {
            ...
            $result = mysql_query("SELECT data, status FROM wordseng WHERE id=$wordid", $link);
            $row = mysql_fetch_array( $result );
            
            if (strlen($row['data'])>100) {
                $fetched = $row['data'];
                
                $updatedata = mysql_query("UPDATE wordseng SET ip_address='$ip', loaded=loaded+1, lasttime='$datetime' WHERE id=$wordid ", $link) ;
            }
            else {
                if ($row['status'] === 'empty') {
                    $cache_status = 'empty';
            ...
            $adddata = mysql_query("INSERT INTO wordseng(ip_address, word, count, loaded, addtime, lasttime) VALUES('$ip', '$slashed_word', 0, 0, '$datetime', '$datetime')", $link);
        }
    ...
    function get_search_handler($link, $type = 'normal') {
        ...
        $result_proxy_query_str =  "SELECT * FROM wordseng_proxy WHERE lasttime<'$datetime_mod' ORDER BY lasttime";
        if ($type === 'active') {
            $result_proxy_query_str =  "SELECT * FROM wordseng_proxy WHERE lasttime<'$datetime_mod' AND sick=0 ORDER BY lasttime";
        }
        ...
        while ($row_proxy = mysql_fetch_array($result_proxy)) {
            ...
            $search_handler = array(
                                    ...
                                    "backup_prefix" => 'http://' . $libgen_host . '/search.php?req=',
                                   // "backup_prefix" => 'http://gen.lib.rus.ec/search.php?req=',
        ...
        if ($results){
            $first = $results[0];
            $id = $first['handler_id'];
            $adddata = mysql_query("UPDATE wordseng_proxy SET lasttime='$datetime' , total=total+1, total_day=total_day+1 WHERE id=$id", $link);
        }
    ...
    function parse_eng($output, &$status, &$log, $str) {
        ...
        foreach ($ret as $value) {
            ...
            $href = 'http://' . $libgen_host . '/' . $l->href;
            // $href = 'http://gen.lib.rus.ec/' . $l->href;
    ...
    function addword($link, $word, $data, $status)
    {
        ...
        if ($row) {
            ...
            $result = mysql_query("SELECT data FROM wordseng WHERE id=$wordid", $link);
            $row = mysql_fetch_array( $result );
            
            if (strlen($row['data']) < 100) {
                if ($status === 'empty') {
                    $updatedata = mysql_query("UPDATE wordseng SET ip_address='$ip', count=count+1, lasttime='$datetime', status='$status'  WHERE id=$wordid ", $link) ;
                } else {
                    $updatedata = mysql_query("UPDATE wordseng SET ip_address='$ip', count=count+1, lasttime='$datetime', data='$slashed_data'  WHERE id=$wordid ", $link) ;
                }
            }
            ...
            if ($status === 'empty') {
                $adddata = mysql_query("INSERT INTO wordseng(ip_address, word, count, loaded, addtime, lasttime, status) VALUES('$ip', '$slashed_word', 0, 0, '$datetime', '$datetime', '$status')", $link);
            } else {
                $adddata = mysql_query("INSERT INTO wordseng(ip_address, word, count, loaded, addtime, lasttime, data) VALUES('$ip', '$slashed_word', 0, 0, '$datetime', '$datetime', '$slashed_data')", $link);
            }
        }
